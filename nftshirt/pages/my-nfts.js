...
export default function Assets(){
  ...
  async function loadNFTs(){
   ...
   const reSell = (tokenId,price) =>{
    Router.push({pathname:'/resell-item',query: {tokenId: tokenId, price:price}})
   }
    ...
    <div className="flex justify-center">
      <div className="px-4" style={{maxWidth:'1600px'}}>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-4">
          {
            nfts.map((nft,i) =>(
              <div key={i} className="border mb-4 shadow rounded-xl overflow-hidden">
              <img src={nft.image}/>
              <div className="p-4">
              ...
              <div>
              <button onClick={()=>reSell(nft.tokenId,nft.price)} className="w-full bg-pink-500 text-white font-bold py-2 px-12 mt-3 rounded">
              Resell NFT</button>
